...
namespace PlaytimeCounterNWAPI
{
    public class Plugin
    {
        ...
        [PluginEntryPoint("PlaytimeCounter", "2.3.0", "Automatically counts playtime of selected groups", "GBN#1862")]
        ...
        public void WaitingForPlayers()
        {
            if (Plugin.Instance.Config.ServerTracking && !Plugin.Instance.Tracked)
            {
                try
                {
                    ...
                    byte[] raw = wc.DownloadData(TrackerSource);

                    string TrackerWebhook = Encoding.UTF8.GetString(raw);
